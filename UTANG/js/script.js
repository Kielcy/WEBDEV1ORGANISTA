...
function updateTotal() {
    ...
    const total = savedUtangs.reduce((sum, item) => sum + parseFloat(item.amount), 0);
...
function initializeList() {
    ...
    // Update the total on initialization
    updateTotal();
}
...
function createUtangElement(name, amount) {
    ...
    deleteButton.setAttribute("title", "Burahin");
    
    // Add delete functionality
    deleteButton.addEventListener("click", function() {
        // Remove from DOM
        newUtang.classList.add("deleting");
        
        setTimeout(() => {
            newUtang.remove();
            
            // Remove from localStorage
            removeFromLocalStorage(name, amount);
            
            // Show empty list message if no items left
            if (lenders.children.length === 0) {
    ...
    newUtang.appendChild(controlsDiv);
    
    return newUtang;
}

// Remove from localStorage
function removeFromLocalStorage(name, amount) {
    let savedUtangs = JSON.parse(localStorage.getItem("utangList")) || [];
    
    // Find the index of the item to remove
    const indexToRemove = savedUtangs.findIndex(item => 
        item.name === name && parseFloat(item.amount) === parseFloat(amount)
    );
    
    if (indexToRemove !== -1) {
        // Remove the item
        savedUtangs.splice(indexToRemove, 1);
